    ...
    while (1) {
            ...
            if (token[1] == NULL) {
                ...
                if (fileCheckBool == 1) {
                    ...
                    for (i = 0; i < 16; i++) {
                        ...
                        int root_offset = (BPB_NumFATs * BPB_FATSz32 * BPB_BytesPerSec) +
                ...
                if (fileCheckBool != 0) {
                    ...
                    for (index = 0; index < 16; index++) {
                        ...
                        if (strcmp(temp_string, text_path) == 0) {
                            //char file[12] =substr(dir[index].DIR_Name,0,strlen(dir[index].DIR_Name));
                           // &(dir[index].DIR_Name[strlen((dir[index].DIR_Name)-1]),
                            
                            printf("FIleName:%s\t Attr:%d\t Low:%d\t Size:%d\n",
                                dir[index].DIR_Name,
                                dir[index].DIR_Attr,
        ...
        //=================================================================================
            ...
            if (token[1] != NULL) {
                ...
                printf("Exception: Cd .. is not implemented yet");
                break;
            }
             else {
        ...
        //=================================================================================
            ...
            if (fileCheckBool == 1) {
                ...
                fread(&value_volume, 1, 11, fp);
                if(strcmp(&value_volume,NULL)==0)
                    printf("Volume name is Empty");
                else
                printf("Volume:*%s*\n", value_volume);
            }
            else {
        ...
        //=================================================================================
             ...
             printf("You are in the get command");
             if(strcmp(token[1],NULL)==0){
                 printf("Error: Command should be provided in the format get <filename>");
             }
             char *filename = token[1];
             char *fileCopy[12];
             int cluster
             ...
             if (closeCheck == 1) {
                 for (i = 0; i < 16; i++) {
                     ...
                     while (size > 512) {
                         fread(filename, 512,  SEEK_SET);
                         fwrite(filename, 512, sizeof(filename), fileCopy);
                         size = size - 512;
                         culster = NextLB();
                     }
            fread(filename, 512, SEEK_SET);
            fwrite(filename,512,1, fileCopy);
         
            }
             else{
                 printf("Error: File must be opended first");
             }
         }
